...
for item in related_list:
    ...
    for genre in item["genres"]:
        target = genre
        genre_connections.append((source, target))

df_genres = pd.DataFrame(genre_connections, columns=["source", "target"])

print(df_genres)

df_genres.to_csv("class3/graf_genres.csv", sep=",", index=False)
